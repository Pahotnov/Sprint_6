...
class TestFillOrderAndClickOnScooterLink:
    @allure.title('Проверка ссылки "Самокат"')
    @allure.description('Переходим по кнопкам "Заказ", заполняем все данные '
                        '(выбираются случайным образом из готовых наборов) и переходим по ссылке "Самокат". '
    ...
    def test_fill_order_and_click_on_scooter_link(self, driver, order_button):
        ...
        ya_scooter_main_page = ya_scooter_order_page.click_on_scooter_link()
